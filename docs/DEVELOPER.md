...
```bash
# Windows (PowerShell)
$env:OTA_PASSWORD="YOUR-OTA-PASSWORD"; pio run --target upload --upload-port YOUR-DEVICE-IP

# Windows (cmd)
python -c "import subprocess,os; env=os.environ.copy(); env['OTA_PASSWORD']='YOUR-OTA-PASSWORD'; subprocess.run(['pio','run','--target','upload','--upload-port','YOUR-DEVICE-IP'], env=env)"

# Linux/macOS
OTA_PASSWORD=YOUR-OTA-PASSWORD pio run --target upload --upload-port YOUR-DEVICE-IP
```
