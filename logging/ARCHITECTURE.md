...
1. ESP32 DEVICE
   ...
   │
   └─ Sends via UDP to 192.168.1.97:9514
...
2. CRIBL STREAM
   ...
   └─ Sends to Elasticsearch via HTTP POST
      Endpoint: http://192.168.1.97:9200/_bulk
...
6. KIBANA DASHBOARD
   ...
   │  URL: http://192.168.1.97:5601/app/dashboards#/view/esp32-dashboard
   │
...
```bash
# Check Cribl status
curl http://192.168.1.97:9000/api/v1/health

# Check Elasticsearch cluster health
curl -u elastic:N5l2MV3c \
  http://192.168.1.97:9200/_cluster/health?pretty

# Check Kibana status
curl -u elastic:N5l2MV3c \
  http://192.168.1.97:5601/api/status
...
# Kibana objects
curl -u elastic:N5l2MV3c \
  http://192.168.1.97:5601/api/saved_objects/_export \
  -H 'kbn-xsrf: true' \
  -H 'Content-Type: application/json' \
...
curl -u elastic:N5l2MV3c \
  -X PUT "http://192.168.1.97:9200/_snapshot/my_backup/snapshot_1?wait_for_completion=true"
```
...
curl -u elastic:N5l2MV3c \
  -X POST "http://192.168.1.97:9200/_snapshot/my_backup/snapshot_1/_restore"
```
