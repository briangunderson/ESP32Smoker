...
- Cribl Stream running on 192.168.1.97:9000
- Elasticsearch running on 192.168.1.97:9200
- Kibana running on 192.168.1.97:5601
...
6. **Configure Your ESP32**
   - Ensure your ESP32 sends syslog to `192.168.1.97:9514` (UDP)
   - Use RFC 5424 format
...
1. Open Kibana: http://192.168.1.97:5601
...
Or direct URL:
```
http://192.168.1.97:5601/app/dashboards#/view/esp32-dashboard
```
...
**Check Cribl:**
1. Go to Cribl Live Data: http://192.168.1.97:9000/monitoring/live-data
2. Filter by source: `syslog_udp`
3. Verify messages are being received

**Check ESP32:**
1. Verify ESP32 is sending to correct IP: 192.168.1.97
2. Verify port: 9514 (UDP)
3. Check firewall allows UDP 9514
4. Test with: `echo "test" | nc -u 192.168.1.97 9514`

**Check Elasticsearch:**
```bash
# Check if indices exist
curl -u elastic:N5l2MV3c \
  http://192.168.1.97:9200/_cat/indices/esp32-logs-*?v

# Check if pipeline exists
curl -u elastic:N5l2MV3c \
  http://192.168.1.97:9200/_ingest/pipeline/esp32-syslog-pipeline
```
...
curl -u elastic:N5l2MV3c \
  "http://192.168.1.97:9200/esp32-logs-*/_search?pretty" \
...
```bash
curl -u elastic:N5l2MV3c \
  -X POST "http://192.168.1.97:9200/_ingest/pipeline/esp32-syslog-pipeline/_simulate?pretty" \
  -H 'Content-Type: application/json' \
  -d '{
    "docs": [{
      "source": {
...
1. **Check watcher status**:
   ```bash
   curl -u elastic:N5l2MV3c \
     "http://192.168.1.97:9200/_watcher/stats?pretty"
   ```

2. **Manually execute watcher**:
   ```bash
   curl -u elastic:N5l2MV3c \
     -X POST "http://192.168.1.97:9200/_watcher/watch/esp32-error-state/_execute?pretty"
   ```

3. **Check watcher history**:
   ```bash
   curl -u elastic:N5l2MV3c \
     "http://192.168.1.97:9200/.watcher-history-*/_search?pretty"
   ```
...
1. **Delete old indices**:
   ...
   curl -u elastic:N5l2MV3c \
     -X DELETE "http://192.168.1.97:9200/esp32-logs-2026.01.01"
   ```
...
```bash
curl -u elastic:N5l2MV3c \
  "http://192.168.1.97:9200/_index_template/esp32-logs" \
...
```bash
curl -u elastic:N5l2MV3c \
  -X PUT "http://192.168.1.97:9200/_index_template/esp32-logs" \
  -H 'Content-Type: application/json' \
...
```bash
# Elasticsearch
export ES_HOST=192.168.1.97
export ES_PORT=9200
export ES_USER=elastic
export ES_PASS=N5l2MV3c

# Kibana
export KIBANA_HOST=192.168.1.97
export KIBANA_PORT=5601

# Cribl
export CRIBL_HOST=192.168.1.97
