...
try {
    $response = Invoke-RestMethod -Uri "$esUrl" -Credential (New-Object PSCredential "elastic", (ConvertTo-SecureString "N5l2MV3c" -AsPlainText -Force))
...
try {
    $response = Invoke-RestMethod -Uri "$esUrl/_ingest/pipeline/esp32-syslog-pipeline" -Credential (New-Object PSCredential "elastic", (ConvertTo-SecureString "N5l2MV3c" -AsPlainText -Force))
    Write-Host " ✓ PASS" -ForegroundColor Green
}
catch {
    Write-Host " ✗ FAIL" -ForegroundColor Red
...
try {
    $response = Invoke-RestMethod -Uri "$esUrl/_index_template/esp32-logs-template" -Credential (New-Object PSCredential "elastic", (ConvertTo-SecureString "N5l2MV3c" -AsPlainText -Force))
    Write-Host " ✓ PASS" -ForegroundColor Green
}
catch {
    Write-Host " ✗ FAIL" -ForegroundColor Red
...
try {
    $response = Invoke-RestMethod -Uri "$kibanaUrl/api/status" -Credential (New-Object PSCredential "elastic", (ConvertTo-SecureString "N5l2MV3c" -AsPlainText -Force))
    Write-Host " ✓ PASS" -ForegroundColor Green
